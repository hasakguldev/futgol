...
class MatchGoalscorerScreen extends StatefulWidget {
  final VoidCallback onBackToMenu;

  const MatchGoalscorerScreen({super.key, required this.onBackToMenu});

  @override
  State<MatchGoalscorerScreen> createState() => _MatchGoalscorerScreenState();
}

class _MatchGoalscorerScreenState extends State<MatchGoalscorerScreen> {
  ...
  Future<void> _loadQuestion() async {
    ...
    var data = await _dbService.getRandomGameWithGoalscorer(_difficulty);
    for (int attempt = 0; data == null && attempt < 3; attempt++) {
      data = await _dbService.getRandomGameWithGoalscorer(_difficulty);
    }
    if (data == null) {
      setState(() {
  ...
  void _submitGuess(String guess) {
    ...
    final String correctAnswer = _questionData!['answer'] as String;
    ...
    if (DbText.namesMatch(guess, correctAnswer)) {
      ...
      setState(() {
        _isWon = true;
        _streak++;
        _feedbackMsg = "TEBRİKLER! Golcü: $correctAnswer";
      });
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_gameStarted) {
      return SoccerGameSetupView(
        gameKey: "match_goalscorer",
        gameTitle: "MAÇ DEDEKTİFİ",
        gameDescription: "Tarihi bir maçın gol dakikaları verilir. Soru işaretiyle gizlenen golü atan oyuncuyu tahmin edin.",
    ...
    final String matchTitle = _questionData!['match_title'] as String;
    final String season = _questionData!['season'] as String;
    final List<dynamic> goalClues = _questionData!['goal_clues'] as List<dynamic>;
                ...
                if (_isGameOver)
                  StateCard(
                    ...
                    additionalWidget: AnswerRevealCard(
                      label: 'CEVAP',
                      playerName: _questionData!['answer'] as String,
                      imageUrl: _questionData!['answer_image'] as String?,
                      facts: ['${_questionData!['target_minute']}. dakika', _questionData!['season']?.toString() ?? ''],
                      color: NeobrutalistColors.pink,
                    ),
                  )
                else if (_isWon)
                  ...
                  NeobrutalistCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          "GOL ANLARI",
                          style: NeobrutalistStyles.headlineStyle(fontSize: 9, color: Colors.grey[800]!),
                          textAlign: TextAlign.center,
                        ),
                        const SizedBox(height: 8),
                        Container(
                          ...
                          child: Column(
                            children: goalClues.map((item) {
                              final isTarget = item.toString().contains("❓");
                              return Container(
                                padding: const EdgeInsets.symmetric(vertical: 4),
                                child: Row(
                                  children: [
                                    Text(isTarget ? "⚽❓" : "⚽", style: const TextStyle(fontSize: 12)),
                                    ...
                                    Expanded(
                                      child: Text(
                                        ...
                                        style: NeobrutalistStyles.headlineStyle(
                                          fontSize: 8.5,
                                          color: isTarget ? NeobrutalistColors.pink : Colors.black,
                                        ),
                                      ),
                                    ),
                                  ],
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        NeobrutalistAutocompleteField(
                          ...
                          hintText: "Soru işaretli golcüyü yazın...",
