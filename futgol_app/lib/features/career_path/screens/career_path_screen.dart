...
class CareerPathScreen extends StatefulWidget {
  ...
  const CareerPathScreen({super.key, required this.onBackToMenu});
...
class _CareerPathScreenState extends State<CareerPathScreen> {
  ...
  Future<void> _loadQuestion() async {
    ...
    var data = await _dbService.getRandomPlayerWithCareer(_difficulty);
    for (int attempt = 0; data == null && attempt < 3; attempt++) {
      data = await _dbService.getRandomPlayerWithCareer(_difficulty);
    }
    if (data == null) {
      setState(() {
  ...
  void _submitGuess(String guess) {
    ...
    if (DbText.namesMatch(guess, correctAnswer)) {
      ...
      setState(() {
        _isWon = true;
        _streak++;
        _feedbackMsg = "TEBRİKLER! Doğru Cevap: $correctAnswer";
      });
    } else {
      ...
      setState(() {
        ...
        if (_lives <= 0) {
          _isGameOver = true;
          _feedbackMsg = "KAYBETTİNİZ! Doğru Cevap: $correctAnswer";
        } else {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_gameStarted) {
      return SoccerGameSetupView(
        gameKey: "career_path",
        gameTitle: "KARİYER YOLU OYUNU",
        gameDescription: "Bir oyuncunun oynadığı kulüpler kronolojik olarak verilir. Bu oyuncunun kim olduğunu tahmin edin.",
    ...
    final String country = _questionData!['country'] as String;
    final String position = _questionData!['position'] as String;
    // Sezon aralıklı etiketler ("Barcelona (2012–2020)") varsa onları kullan;
    // yoksa sade kulüp listesine düş.
    final List<dynamic> career =
        (_questionData!['career_labels'] as List<dynamic>?) ??
            (_questionData!['career'] as List<dynamic>);
                ...
                if (_isGameOver)
                  StateCard(
                    ...
                    additionalWidget: AnswerRevealCard(
                      ...
                      facts: [position, country, if ((_questionData!['market_value'] as int? ?? 0) > 0) FootballPlayer.formatEuro(_questionData!['market_value'] as int)],
                      color: NeobrutalistColors.pink,
                    ),
                  )
                else if (_isWon)
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Chip(
                              label: Text("Mevki: $position", style: NeobrutalistStyles.headlineStyle(fontSize: 8, color: Colors.white)),
                              backgroundColor: NeobrutalistColors.blue,
                            ),
                            const SizedBox(width: 8),
                            Chip(
                              label: Text("Ülke: $country", style: NeobrutalistStyles.headlineStyle(fontSize: 8, color: Colors.white)),
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(
                                ...
                                leading: CircleAvatar(
                                  backgroundColor: NeobrutalistColors.yellow,
                                  radius: 12,
                                  child: Text("${index + 1}", style: const TextStyle(fontSize: 10, color: Colors.black, fontWeight: FontWeight.bold)),
                                ),
                                title: Text(
                                  item.toString(),
