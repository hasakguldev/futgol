...
class MarketValueQuizScreen extends StatefulWidget {
  final VoidCallback onBackToMenu;

  const MarketValueQuizScreen({super.key, required this.onBackToMenu});

  @override
  State<MarketValueQuizScreen> createState() => _MarketValueQuizScreenState();
}

class _MarketValueQuizScreenState extends State<MarketValueQuizScreen> {
  ...
  Future<void> _loadQuestion() async {
    ...
    var data = await _dbService.getRandomPlayerValuationQuiz(_difficulty);
    for (int attempt = 0; data == null && attempt < 3; attempt++) {
      data = await _dbService.getRandomPlayerValuationQuiz(_difficulty);
    }
    if (data == null) {
      setState(() {
  ...
  void _submitGuess(String option) {
    ...
    setState(() {
      ...
      if (correct) {
        ...
        _feedbackMsg = "DOĞRU! ${_questionData!['player_name']} zirve piyasa değeri gerçekten $correctAnswer.";
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_gameStarted) {
      return SoccerGameSetupView(
        gameKey: "market_value_quiz",
        gameTitle: "PİYASA DEĞERİ TAHMİN",
        gameDescription: "İsmi, kulübü ve pozisyonu verilen oyuncunun ulaştığı en yüksek piyasa değerini 4 seçenek arasından bulun.",
    ...
    final String name = _questionData!['player_name'] as String;
