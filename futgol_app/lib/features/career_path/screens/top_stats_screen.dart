...
class TopStatsScreen extends StatefulWidget {
  final VoidCallback onBackToMenu;

  const TopStatsScreen({super.key, required this.onBackToMenu});

  @override
  State<TopStatsScreen> createState() => _TopStatsScreenState();
}

class _TopStatsScreenState extends State<TopStatsScreen> {
  ...
  Future<void> _loadQuestion() async {
    ...
    var data = await _dbService.getRandomTopStatsQuiz(_difficulty);
    for (int attempt = 0; data == null && attempt < 3; attempt++) {
      data = await _dbService.getRandomTopStatsQuiz(_difficulty);
    }
    if (data == null) {
      setState(() {
  ...
  void _submitGuess(String guess) {
    ...
    if (DbText.namesMatch(guess, correctAnswer)) {
      ...
      setState(() {
        _isWon = true;
        _streak++;
        _feedbackMsg = "TEBRİKLER! Zirve İstatistik Sahibi: $correctAnswer";
      });
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_gameStarted) {
      return SoccerGameSetupView(
        gameKey: "top_stats",
        gameTitle: "ZİRVE İSTATİSTİKLER",
        gameDescription: "Verilen sezonda ve turnuvada belirtilen sayıda gol atarak gol kralı/zirve performans sahibi olmuş oyuncuyu tahmin edin.",
    ...
    final int goals = _questionData!['goals'] as int;
    final String competition = _questionData!['competition'] as String;
                ...
                if (_isGameOver)
                  StateCard(
                    ...
                    additionalWidget: AnswerRevealCard(
                      ...
                      facts: ['⚽ ${_questionData!['goals']}', _questionData!['competition']?.toString() ?? '', _questionData!['season']?.toString() ?? ''],
                      color: NeobrutalistColors.pink,
                    ),
                  )
                else if (_isWon)
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.stretch,
                            children: [
                              Text("Sezon: $season", style: NeobrutalistStyles.headlineStyle(fontSize: 9)),
                              const SizedBox(height: 4),
                              Text("Turnuva: $competition", style: NeobrutalistStyles.headlineStyle(fontSize: 9)),
                              const SizedBox(height: 4),
                              Text("Başarı: Toplam $goals Gol ⚽", style: NeobrutalistStyles.headlineStyle(fontSize: 9, color: NeobrutalistColors.pink)),
                            ],
                          ),
                        ),
                      ],
